...
so the caller always knows exactly where in the pipeline a failure occurred.
"""
...
def run_pipeline(question: str, language: str = "en") -> dict:
    ...
    Args:
        ...
        language: Language code for Agent 3's narrative ('en', 'es', 'de', ...).
        ...
        On success:
            {
                ...
                "downloadable_csv": str | None,
                "agent1_meta":     { tables_used, filters_applied, confidence_score }
            }
        On failure:
            { "status": "error", "stage": str, "message": str }
    """

    # ── Fetch schema once — shared by Agent 1 (text) and Agent 2 (dict) ───────
    try:
    ...
    try:
        agent1 = generate_sql(
            question,
            context={"schema": combined_schema, "business_rules": BUSINESS_RULES}
        )
        if "error" in agent1:
            return {
                "status":  "error",
    ...
    # ── Stage 2: Agent 2 — validate + dialect repair ──────────────────────────
    try:
    ...
    # ── Stage 4: Agent 3 — format + narrate ───────────────────────────────────
    try:
    ...
    # ── Stage 5: S3 export (non-fatal — pipeline succeeds even if this fails) ─
    csv_url = None
    if rows:
        try:
